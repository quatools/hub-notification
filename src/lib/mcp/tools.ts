  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        recipient: { type: 'string', enum: ['member', 'fixed'], description: "MP Discord ET Email : 'member' (membre concerné, défaut) ou 'fixed' (personne/adresse précise)" },
        discord_user_id: { type: 'string', description: 'ID Discord (type discord_dm, uniquement si recipient=fixed)' },
        email: { type: 'string', description: 'Adresse email (type email, uniquement si recipient=fixed)' },
      },
      required: ['type'],
    },
  },
